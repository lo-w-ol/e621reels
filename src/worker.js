...
function renderPhotoGridPage() {
  ...
  function photoTagsFromUrl(){const url=new URL(window.location.href);const raw=(url.searchParams.get('tags')||'').trim().split(/\\s+/).filter(Boolean);const rating=(url.searchParams.get('rating')||'').trim().toLowerCase();const ratio=(url.searchParams.get('ratio')||'').trim().toLowerCase();const tags=['order:score',...raw];if(rating==='s'||rating==='q'||rating==='e')tags.push('rating:'+rating);if(ratio==='vertical')tags.push('ratio:<1');if(ratio==='landscape')tags.push('ratio:>1');return tags.join(' ');}
  function mapApiPost(post){return {id:post.id,mediaUrl:post.file&&post.file.url?post.file.url:'',previewUrl:(post.preview&&post.preview.url)||(post.sample&&post.sample.url)||(post.file&&post.file.url)||'',ext:String(post.file&&post.file.ext?post.file.ext:'').toLowerCase()};}
